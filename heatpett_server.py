...
class App(tk.Tk):
    def __init__(self):
        ...
        self._d_thumb_r      = tk.IntVar(value=self._cfg.get("d_thumb_r",      6))
        self._d_seg_h        = tk.IntVar(value=self._cfg.get("d_seg_h",       26))
        self._d_row_pad      = tk.IntVar(value=self._cfg.get("d_row_pad",      8))
    ...
    def _save_config(self):
        cfg = {
            ...
            "d_thumb_r":      self._d_thumb_r.get(),
            "d_seg_h":        self._d_seg_h.get(),
            "d_row_pad":      self._d_row_pad.get(),
        }
        try:
    ...
    def _build_main_card(self):
        ...
        tr = self._d_thumb_r.get()
        sh = self._d_seg_h.get()
        rp = self._d_row_pad.get()
        ...
        status.pack(fill="x", padx=20, pady=(rp + 6, rp))
        ...
        int_label_row.pack(fill="x", padx=20, pady=(rp + 2, 2))
        ...
        FancySlider(card, variable=self._int_var, from_=0, to=100,
                    ...
                    track_h=4, thumb_r=tr, p_bg=BG
                    ).pack(fill="x", padx=16, pady=(0, rp))

        # ── Mode row ──────────────────────────────────────────────────────────
        mode_row = tk.Frame(card, bg=BG)
        mode_row.pack(fill="x", padx=20, pady=(rp, rp))
        ...
        seg = SegmentedControl(mode_row, ["Proximity", "Trigger"],
                               ...
                               seg_w=84, h=sh, r_cont=7, r_seg=6, pad=3, p_bg=BG)
        seg.pack(side="right")

        # ── Test row ──────────────────────────────────────────────────────────
        test_row = tk.Frame(card, bg=BG)
        test_row.pack(fill="x", padx=20, pady=(rp, rp + 6))
